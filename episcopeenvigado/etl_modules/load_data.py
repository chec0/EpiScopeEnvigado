...
def preparacion_dataset(df):
    ...
    # =========================
    # 4) CONSTRUCCIÓN DE DIMENSIONES (con IDs sustitutos)
    # =========================

    # --- dim_via_ingreso ---
    dim_via = (
        df[['VIA INGRESO']].dropna().drop_duplicates().sort_values(by='VIA INGRESO')
        .rename(columns={'VIA INGRESO':'via_ingreso_cod'})
        .assign(via_ingreso_desc=lambda d: d['via_ingreso_cod'].astype('Int64').map(CAT_VIA_INGRESO))
    ...
    dim_estado = (
        df[['Estado_Salida']].fillna('NO_INFO').drop_duplicates().sort_values(by='Estado_Salida')
        .rename(columns={'Estado_Salida':'estado_salida_cod'})
        .assign(estado_salida_desc=lambda d: d['estado_salida_cod'])  # puedes mapear a cat oficial si lo tienes
        .reset_index(drop=True)
    )
    ...
    # --- dim_causa_ext ---
    dim_causa = (
        df[['CAUSA EXT']].dropna().drop_duplicates().sort_values(by='CAUSA EXT')
        .rename(columns={'CAUSA EXT':'causa_ext_cod'})
        .assign(causa_ext_desc=lambda d: d['causa_ext_cod'].astype('Int64').map(CAT_CAUSA_EXT))
        .reset_index(drop=True)
    )
    ...
    # --- dim_departamento ---
    dim_depto = (
        df[['DEPARTAMENTO']].dropna().drop_duplicates().sort_values(by='DEPARTAMENTO')
    ...
    dim_muni = (
        df[['MUNICIPIO_DANE','DEPARTAMENTO','MUNICIPIO']].dropna().drop_duplicates().sort_values(by='MUNICIPIO_DANE')
        .rename(columns={'MUNICIPIO_DANE':'municipio_dane',
    ...
    dim_edad = (
        df[['EDAD','UNIDAD EDAD','UNIDAD_EDAD_TXT']].drop_duplicates().sort_values(by=['UNIDAD EDAD','EDAD'])
        .rename(columns={'UNIDAD EDAD':'unidad_edad_cod'})
        .assign(edad_anios=lambda d: d.apply(lambda r: edad_a_anios(r['EDAD'], r['UNIDAD_EDAD_TXT']), axis=1))
        .reset_index(drop=True)
    )
    ...
    map_estado = dict(zip(dim_estado['estado_salida_cod'], dim_estado['estado_salida_id']))
    ...
    dim_edad['key'] = list(zip(dim_edad['EDAD'].astype('Int64'), dim_edad['unidad_edad_cod'].astype('Int64')))
